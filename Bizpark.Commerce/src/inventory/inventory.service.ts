...
export class InventoryService {
  ...
  async upsert(
    tenantId: string,
    payload: { productId: string; sku: string; availableQuantity: number; reservedQuantity?: number; variantId?: string },
  ) {
    const repo = await this.repo(tenantId);
    ...
    if (existing) {
      ...
      if (payload.variantId !== undefined) existing.variantId = payload.variantId ?? null;
  ...
  /**
   * Reserve by productId or variantId — with pessimistic write lock (prevents race conditions).
  ...
  async reserveWithLock(
    ...
    if (!item) return { success: true }; // untracked product → allow

    const qty = Math.max(1, Number(quantity));
    if (item.availableQuantity < qty) {
      return { success: false, message: `Insufficient stock for product ${productId}${variantId ? ` / variant ${variantId}` : ''}` };
    }
  ...
  /**
   * Release reserved inventory — with pessimistic write lock.
   * Must be called inside an active QueryRunner transaction.
   */
  ...
  /** Non-transactional reserve by productId (used outside transactions) */
  ...
  /** Non-transactional release by productId */
